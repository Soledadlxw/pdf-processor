...
6. **`_chunks/`** 目录为 RAG 子文档，切分逻辑统一在 `notes.py`：
   - 每块都是父笔记正文的**逐字切片**（子文档命中、父文档供上下文）
   - 每块都不超过 `child_chunk_size`。优先切在 `##` 边界，小节超预算再依次降级到 `###`、空行分段，最后才硬切
   - 只切得出一块时不写子文档——那会是父笔记的复制品，检索时等于同一段内容命中两次
   - frontmatter 里的 `sections` 记录本块覆盖的小节名，便于单独命中时判断上下文
